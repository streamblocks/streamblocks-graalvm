...
/**
 * The CAL implementation of {@link NodeLibrary} provides fast access to local variables. It's used
...
public abstract class CALScopedNode extends Node {

    /**
     * Index to the the {@link CALBlockNode#getDeclaredLocalVariables() block's variables} that
     ...
     */
    @ExportMessage
    boolean accepts(@Shared("node") @Cached(value = "this", adopt = false) CALScopedNode cachedNode) {
    ...
    @ExportMessage
    @SuppressWarnings("static-method")
    final Object getScope(Frame frame, boolean nodeEnter, @Shared("node") @Cached(value = "this", adopt = false) CALScopedNode cachedNode,
                    @Cached(value = "this.findBlock()", adopt = false, allowUncached = true) Node blockNode) {
        if (blockNode instanceof StmtBlockNode) {
            return new VariablesObject(frame, cachedNode, nodeEnter, (StmtBlockNode) blockNode);
        } else {
            return new ArgumentsObject(frame, (CALRootNode) blockNode);
        }
    }
    ...
    @ExportMessage
    @TruffleBoundary
    final boolean hasRootInstance(@SuppressWarnings("unused") Frame frame, @CachedContext(CALLanguage.class) ContextReference<CALContext> contextRef) {
        String functionName = getRootNode().getName();
        CALContext context = contextRef.get();
    ...
    @ExportMessage
    @TruffleBoundary
    final Object getRootInstance(@SuppressWarnings("unused") Frame frame, @CachedContext(CALLanguage.class) ContextReference<CALContext> contextRef) throws UnsupportedMessageException {
        String functionName = getRootNode().getName();
        CALContext context = contextRef.get();
        // The instance of the current RootNode is a function of the same name.
    ...
    /**
     * Find block of this node. Traverse the parent chain and find the first {@link CALBlockNode}. If
     * none is found, {@link RootNode} is returned.
     *
     * @return the block node, always non-null. Either CALBlockNode, or CALRootNode.
     */
    public final Node findBlock() {
        Node parent = getParent();
        while (parent != null) {
            if (parent instanceof StmtBlockNode) {
                break;
            }
    ...
    /**
     * Set the index to the the {@link CALBlockNode#getDeclaredLocalVariables() block's variables}
    ...
    /**
     * Scope of function arguments. This scope is provided by nodes just under a {@link CALRootNode},
     * outside of a {@link CALBlockNode block}.
     * <p>
     * The arguments declared by {@link CALRootNode#getDeclaredArguments() root node} are provided as
     * scope members.
     */
    ...
    static final class ArgumentsObject implements TruffleObject {
        ...
        protected final CALRootNode root;

        /**
         * The arguments depend on the current frame and root node.
         */
        ArgumentsObject(Frame frame, CALRootNode root) {
            this.frame = frame;
            this.root = root;
        }
         ...
         */
        @ExportMessage
        boolean accepts(@Cached(value = "this.root", adopt = false) CALRootNode cachedRoot) {
    ...
    static final class VariablesObject implements TruffleObject {
        ...
        protected final CALScopedNode node;  // the current node
        final boolean nodeEnter;            // whether the node was entered or is about to be exited
        protected final StmtBlockNode block;  // the inner-most block of the current node

        VariablesObject(Frame frame, CALScopedNode node, boolean nodeEnter, StmtBlockNode blockNode) {
            this.frame = frame;
            this.node = node;
         ...
         */
        @ExportMessage
        boolean accepts(@Cached(value = "this.node", adopt = false) CALScopedNode cachedNode,
        ...
        Object toDisplayString(boolean allowSideEffects, @Shared("block") @Cached(value = "this.block", adopt = false) StmtBlockNode cachedBlock,
            ...
            if (parentBlock instanceof StmtBlockNode) {
                ...
                return ((CALRootNode) parentBlock).getName();
            }
        }
        ...
        boolean hasScopeParent(
                        @Shared("block") @Cached(value = "this.block", adopt = false) StmtBlockNode cachedBlock,
                        @Shared("parentBlock") @Cached(value = "this.block.findBlock()", adopt = false, allowUncached = true) Node parentBlock) {
            // Cache the parent block for the fast-path access
            return parentBlock instanceof StmtBlockNode;
        }
        ...
        Object getScopeParent(
            ...
            return new VariablesObject(frame, cachedBlock, true, (StmtBlockNode) parentBlock);
        }
        ...
        SourceSection getSourceLocation() {
            ...
            if (parentBlock instanceof RootNode) {
                ...
                assert parentBlock instanceof CALRootNode : String.format("In CALLanguage we expect CALRootNode, not %s", parentBlock.getClass());
                return ((CALRootNode) parentBlock).getSourceSection();
            } else {
