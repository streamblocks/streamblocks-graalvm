...
public final class StmtBlockNode extends CALStatementNode implements BlockNode.ElementExecutor<CALStatementNode> {
     ...
     * bailout.
     */
    @Child private BlockNode<CALStatementNode> block;
    ...
    public StmtBlockNode(CALStatementNode[] bodyNodes) {
        /*
     ...
     * {@link CALStatementNode#executeVoid} method of all children to be inlined.
     */
    @Override
     ...
     * startsWith semantics if needed. For CAL we don't need to pass any argument as we just have
    ...
    private CALWriteFrameSlotNode[] findDeclaredLocalVariables() {
        ...
        NodeUtil.forEachChild(block, new NodeVisitor() {
            ...
            public boolean visit(Node node) {
                ...
                if (node instanceof CALScopedNode) {
                    CALScopedNode scopedNode = (CALScopedNode) node;
                    scopedNode.setVisibleVariablesIndexOnEnter(varsIndex[0]);
                }
                // Do not enter any nested blocks.
                if (!(node instanceof StmtBlockNode)) {
                    NodeUtil.forEachChild(node, this);
                }
