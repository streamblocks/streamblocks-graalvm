 ...
 * "evaluating" source code in CAL.</li>
 * <li>Conversion of arguments to types understood by CAL. The CAL source code can be evaluated from a
 * different language, i.e., the caller can be a node from a different language that uses types not
 * understood by CAL.</li>
 * </ul>
 */
public final class CALEvalRootNode extends RootNode {
    ...
    public Object execute(VirtualFrame frame) {
        if (language.isSingleContext()) {
             ...
             * multi-context case is initialized with CALLanguage#initializeMultipleContexts. That
        ...
        if (mainCallNode == null) {
            ...
            /* Conversion of arguments to types understood by CAL. */
            Object[] arguments = frame.getArguments();
            for (int i = 0; i < arguments.length; i++) {
                arguments[i] = CALContext.fromForeignValue(arguments[i]);
            }
