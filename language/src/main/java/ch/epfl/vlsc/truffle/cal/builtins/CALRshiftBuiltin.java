 ...
 */
@NodeInfo(shortName = "rshift")
public abstract class CALRshiftBuiltin extends CALBuiltinNode {

    @Specialization
    @TruffleBoundary
    public Object rshift(CALBigNumber value, CALBigNumber amount,
                    @CachedLibrary(limit = "3") InteropLibrary interop,
                    @CachedContext(CALLanguage.class) CALContext context) {
        return new CALBigNumber(value.getValue().shiftRight(amount.getValue().intValue()));
    }
