...
public class EntityListComprehensionVisitor extends CALParserBaseVisitor {
    private final String tempListName;
    private final String listIndexVarName;

    public EntityListComprehensionVisitor(String tempListName, String listIndexVarName) {
        this.tempListName = tempListName;
        this.listIndexVarName = listIndexVarName;
    }

    public CALStatementNode visitEntityListComprehension(CALParser.EntityListExpressionContext ctx) {
        ScopeEnvironment.getInstance().pushScope(false, true);
        CALStatementNode nestedComprehensions = createNestedEntityComprehensions(ctx, 0);
    ...
    private CALStatementNode createNestedEntityComprehensions(CALParser.EntityListExpressionContext ctx, int i) {
        int n = ctx.generators().generator().size();

        if (n == i) {
            ArrayList<CALStatementNode> listWrites = new ArrayList<>();
            listWrites.ensureCapacity(2 * ctx.entityExpressions().entityExpression().size());
            for(CALParser.EntityExpressionContext computation : ctx.entityExpressions().entityExpression()) {
                CALExpressionNode expr = EntityVisitor.getInstance().visitEntityExpression(computation);
        ...
        CALStatementNode ifBody = createNestedEntityComprehensions(ctx, i + 1);
